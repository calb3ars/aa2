...
class MyHashSet
  ...
  def union(set2)
    ...
    self.to_a.each {|key| result.insert(key)}
    set2.to_a.each {|key2| result.insert(key2)}
  ...
  def minus(set2)
    result = self.class.new
    self.to_a.each {|key| result.insert(key) unless set2.include?(key)}
  ...
  def symmetric_difference(set2)
    result = self.class.new
    self.to_a.each {|key| result.insert(key) unless set2.include?(key)}
    set2.to_a.each {|key| result.insert(key) unless self.include?(key)}

    result
  end

  def ==(object)
    return false if !object.is_a?(MyHashSet) || object.to_a.length != @store.to_a.length
    object.to_a.all? do |object_el|
      @store.to_a.include?(object_el)
    end
  end
end
