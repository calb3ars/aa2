...
class String
  def caesar(shift)
    translated_letters = self.split("").map do |letter|
      ((letter.ord - 97 + shift) % 26 + 97).chr
    end
    translated_letters.join("")
  end
end
...
class Hash
  def difference(other_hash)
    ...
    self.each do |key, value|
      results[key] = value unless other_hash.include?(key)
    end

    other_hash.each do |key2, value2|
      results[key2] = value2 unless self.include?(key2)
    end
