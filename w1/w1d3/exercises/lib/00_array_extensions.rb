...
class Array
  def two_sum
    ...
    self.each_with_index do |el, idx|
      ((idx + 1)...self.length).to_a.each do |idx2|
...
class Array
  def my_transpose
    new_height = self.first.count
    new_width = self.length

    results = Array.new(new_height) {Array.new(new_width)}

    new_width.times do |i|
      new_height.times do |j|
